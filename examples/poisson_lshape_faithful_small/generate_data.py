from __future__ import annotations

import argparse
from pathlib import Path

import numpy as np


def _in_lshape(points: np.ndarray) -> np.ndarray:
    return ~((points[:, 0] > 0.0) & (points[:, 1] > 0.0))


def target_solution(xy: np.ndarray) -> np.ndarray:
    x = xy[:, 0]
    y = xy[:, 1]
    theta = np.mod(np.arctan2(y, x), 2.0 * np.pi)
    r = np.sqrt(x * x + y * y) + 1e-6
    singular = np.power(r, 2.0 / 3.0) * np.sin(2.0 * theta / 3.0)
    smooth = 0.15 * np.sin(np.pi * x) * np.sin(np.pi * y)
    return (singular + smooth).reshape(-1, 1)


def forcing_term(xy: np.ndarray) -> np.ndarray:
    x = xy[:, 0]
    y = xy[:, 1]
    # The singular wedge term is harmonic away from the corner. The smooth
    # component contributes -Delta u = 0.3*pi^2*sin(pi*x)*sin(pi*y).
    forcing = 0.3 * np.pi**2 * np.sin(np.pi * x) * np.sin(np.pi * y)
    return forcing.reshape(-1, 1)


def _sample_lshape(rng: np.random.Generator, n: int) -> np.ndarray:
    samples: list[np.ndarray] = []
    while sum(len(part) for part in samples) < n:
        candidate = rng.uniform(-1.0, 1.0, size=(max(n, 256), 2))
        samples.append(candidate[_in_lshape(candidate)])
    return np.concatenate(samples, axis=0)[:n]


def _boundary_points(n_per_edge: int) -> np.ndarray:
    t = np.linspace(-1.0, 1.0, n_per_edge)
    nonpos = t[t <= 0.0]
    nonneg = t[t >= 0.0]
    outer = [
        np.column_stack([np.full_like(t, -1.0), t]),
        np.column_stack([t, np.full_like(t, -1.0)]),
        np.column_stack([np.full_like(nonpos, 1.0), nonpos]),
        np.column_stack([nonpos, np.full_like(nonpos, 1.0)]),
    ]
    notch = [
        np.column_stack([np.zeros_like(nonneg), nonneg]),
        np.column_stack([nonneg, np.zeros_like(nonneg)]),
    ]
    points = np.concatenate([*outer, *notch], axis=0)
    return np.unique(points, axis=0)


def generate(seed: int, output_dir: Path) -> None:
    rng = np.random.default_rng(seed)
    output_dir.mkdir(parents=True, exist_ok=True)

    x_train = _sample_lshape(rng, 900)
    u_train = target_solution(x_train)

    x_boundary = _boundary_points(56)
    u_boundary = target_solution(x_boundary)

    x_residual = _sample_lshape(rng, 1400)
    f_residual = forcing_term(x_residual)

    axis = np.linspace(-1.0, 1.0, 58)
    xx, yy = np.meshgrid(axis, axis)
    grid = np.column_stack([xx.ravel(), yy.ravel()])
    x_val = grid[_in_lshape(grid)]
    u_val = target_solution(x_val)

    np.savez(
        output_dir / "train_data.npz",
        x_train=x_train,
        u_train=u_train,
        x_boundary=x_boundary,
        u_boundary=u_boundary,
        x_residual=x_residual,
        f_residual=f_residual,
    )
    np.savez(output_dir / "val_data.npz", x_val=x_val, u_val=u_val)


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--seed", type=int, default=0)
    parser.add_argument("--output-dir", type=Path, default=Path("."))
    args = parser.parse_args()
    generate(args.seed, args.output_dir)


if __name__ == "__main__":
    main()
