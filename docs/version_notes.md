...
- trace node reference audit counters：`trace_summary.json` 输出 allowlist 中被检查的 solution-reference event 数、被跳过的非 solution event 数，以及两者按 event name 聚合的分布；exported run 若有最终 node set 但没有任何 checked solution-reference event，会使 quality gate fail closed。
