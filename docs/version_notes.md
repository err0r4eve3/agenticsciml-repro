...
- 前端 Wiki 索引页显示双语论文-问题案例，右侧 JSON textarea 可手动修正、保存到当前
  account namespace、复制和重新生成。手动编辑不写入 run artifact 或 evaluator 事实层。
...
- High-Frequency Scaling spectral-bias 论文现在优先落到 convolutional neural operator 的 HFS
  高频失效缓解审计，并保留单相/两相流体与 diffusion-conditioned baseline 边界；不会再因为摘要中的
  diffusion/turbulence 对照段落被误归为 diffusion-corrected turbulence rollout 问题。
...
- In-context operator spectral audit 论文现在优先落到 Jacobian / tangent-operator 的 local PDE
  mechanism fidelity、stability、sensitivity 和 prompt consistency 审计，而不是 generic spectral-bias
  mitigation 问题。
- UFO / Domain-Unification-Free Operator 论文现在优先落到 cross-domain neural operator 与
  discretization-decoupled generalized operator learning 审计，而不是被 high-frequency 字样误归为
...
- Physics-Informed Neural Networks and Extensions 现在落到 PINN review/extensions 与
  governing-equation discovery 的覆盖审计，并保留 residual formulation、practical extension、
...
- FMEnets / plug-flow reactor 论文现在落到 physics-informed Flow-Material-Energy reactor design
  审计，并保留 Navier-Stokes、material/energy balance、sparse inverse measurement 和 finite-element
  对比边界；不再误归为 stiff chemical kinetics neural-operator surrogate。
- Geometry-aware neural preconditioner 论文现在落到 hybrid iterative parametric-PDE solver 审计，
  并保留 Geo-DeepONet、unstructured-mesh geometry transfer、relaxation/Krylov coupling、鲁棒性和效率边界；
  不再退回 generic learned linear-solver aid。
- NSPOD 论文现在落到 DeepONet-learned POD preconditioner / Krylov linear solver 审计，并保留
  multigrid-like subspace、unstructured CAD geometry、solid-mechanics PDE、AMG 对比和收敛加速边界；
  Geo-DeepONet related-work 句子不会再把它误归为 geometry-aware preconditioner 论文。
