...
- trace artifact consistency gate：`trace_summary.json` 会检查 `evaluation_contract.json`、`run_metadata.json` 和 workflow-start trace 中的 fidelity/evidence 字段一致性；不一致时 quality gate fail closed。
